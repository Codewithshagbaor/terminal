...
export default function TerminalPage() {
    ...
    return (
        <div className="min-h-screen bg-slate-50 dark:bg-[#050508] py-4 sm:py-8 px-0 sm:px-4 md:px-8">
            <div className="w-full sm:max-w-4xl mx-auto">
                <div className="space-y-4 sm:space-y-6 animate-in fade-in zoom-in-95 duration-300 relative transition-colors">
                    ...
                    {/* Main Terminal UI */}
                    <div className="obsidian-card rounded-none sm:rounded-2xl overflow-hidden shadow-lg dark:shadow-2xl transition-all duration-500 bg-white dark:bg-transparent border-slate-200 dark:border-white/10">
                        <div className="p-4 sm:p-5 md:p-6 border-b border-slate-100 dark:border-white/5 bg-slate-50/50 dark:bg-white/2 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 sm:gap-4">
                            <div className="flex items-center gap-3">
                                <div className="w-10 h-10 rounded-lg bg-emerald-500/10 border border-emerald-500/20 flex items-center justify-center shrink-0">
                                    <Shield className="w-5 h-5 text-emerald-600 dark:text-emerald-400" />
                                </div>
                                <div>
                                    <h2 className="font-mono text-sm font-bold text-slate-900 dark:text-slate-200 tracking-tight">BET-{betId}</h2>
                            ...
                            <div className="text-2xl font-bold font-mono text-slate-900 dark:text-white tracking-tighter">
                                {formattedAmount} <span className="text-xs text-slate-400 dark:text-slate-500 uppercase font-normal">{tokenSymbol}</span>
                            </div>
                        </div>
                                ...
                                <div className="flex flex-wrap items-center justify-center gap-6 sm:gap-12 py-4">
                                    {participants?.map((participant, idx) => (
                                        <Combatant
                                            ...
                                            isWinner={isSettled && decodedOutcome === participant}
                                            isLoser={isSettled && decodedOutcome !== participant}
                                        />
                                    ))}
                                </div>
                            </div>
